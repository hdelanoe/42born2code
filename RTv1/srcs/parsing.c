...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/01/31 14:49:28 by hdelanoe          #+#    #+#             */
/*   Updated: 2018/01/31 14:49:29 by hdelanoe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		create_environment(t_datas *d, char *line)
{
	...
	if (!d->camera.origin.x && !d->camera.origin.y && !d->camera.origin.z)
		ft_kill("Camera can not be set in [0, 0, 0]");
	i = 0;
	while (tab_line[i])
	{
		free(tab_line[i]);
...
void		open_close(char *src_file)
{
	...
	close(fd);
	if (!(i))
		ft_kill("Bad file. Please check the arguments you try to read.");
}
