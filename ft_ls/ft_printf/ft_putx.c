...
static void	accu_fld(t_env *e, t_buff *buff, char *ox)
{
...
static void	minus(t_env *e, t_buff *buff, char *ox)
{
	if (e->flag[0] > 0)
	{
		ft_putstr(ox);
...
static void	ft_convert_arg(va_list arg, t_env *e, int up, t_buff *buff)
{
	if (e->flag[5] >= 2)
	{
		if (e->flag[5] > 1)
